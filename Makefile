...
rq-worker-start: 
	$(PYTHON) -m jrp.services.rq_worker rq-worker io:$(NUM_RQ_WORKER_IO) computation:$(NUM_RQ_WORKER_COMPUTATION)

rq-worker-fail-recover: 
	$(PYTHON) -m jrp.services.rq_worker rq-failure-recover io computation
...
arxiv-db-update-daemon:
	while true; do make arxiv-db-update; sleep 3600; done 
